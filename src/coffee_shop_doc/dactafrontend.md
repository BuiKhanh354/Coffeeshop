...
👤 Khách hàng:
Giỏ hàng → Checkout → Nhập thông tin → Chọn COD → Đặt hàng
         ↓
[Transaction: Validate stock → Create Order → Trừ tồn kho → Create Payment]
         ↓
Success page (OrderCode, TotalAmount, PaymentMethod)

👨‍💼 Admin xử lý (/Admin/Orders):
Click đơn hàng → Modal chi tiết → Chọn trạng thái → Cập nhật
         ↓
New → Processing → Shipping → Delivered
                              ↓
                   [Auto: PaymentStatus='Paid', Payment.PaidAt=NOW]